...
是不正确的，因为`Hello`和`World`是两个不用的参数。这个问题由组合来解决。
...
那么，Tcl如此动态，你猜怎么着？它或多或少和当前的Ruby实现一样快速。Tcl
...
使用Tcl你可以混合编写面向对象代码，函数式代码，和命令式代码，或多或少像
...
如果你定义了一个过程叫做`unknown`，这个过程将会在Tcl处理命令出错时，将
